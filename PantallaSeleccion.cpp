...
void PantallaSeleccion::procesarNiveles(sf::Event& evento, Juego& juego) {
    if (evento.type == sf::Event::MouseButtonPressed && evento.mouseButton.button == sf::Mouse::Left) {
        if (botonPresionado(240, 155, 320, 65)) {
            juego.iniciar(Nivel::UNO);
        }
        if (botonPresionado(240, 253, 320, 65)) {
            juego.iniciar(Nivel::DOS);
        }
        if (botonPresionado(240, 351, 320, 65)) {
            juego.iniciar(Nivel::TRES);
        }
        if (botonPresionado(270, 445, 260, 42)) {
            mostrarEstampillas = true;
        }
        if (botonPresionado(290, 500, 220, 42)) {
