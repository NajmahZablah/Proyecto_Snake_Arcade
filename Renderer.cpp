...
Renderer::Renderer(int anchoVentana, int altoVentana)
    : ventana(sf::VideoMode(anchoVentana, altoVentana), "Snake Arcade",
...
bool Renderer::ventanaAbierta() const { return ventana.isOpen(); }
void Renderer::cerrar()               { ventana.close(); }
...
void Renderer::cargarTexturas() {
    if (!texFondoMenu.loadFromFile("assets/fondo_menu.png"))           cerr << "Error: fondo_menu.png\n";
    if (!texFondoSeleccion.loadFromFile("assets/fondo_seleccion.png")) cerr << "Error: fondo_seleccion.png\n";
    if (!texFondoJuego.loadFromFile("assets/fondo_juego.png"))         cerr << "Error: fondo_juego.png\n";
    if (!texAceituna.loadFromFile("assets/aceituna.png"))               cerr << "Error: aceituna.png\n";
    if (!texAceitunaDorada.loadFromFile("assets/aceituna_dorada.png")) cerr << "Error: aceituna_dorada.png\n";
    if (!texCaja.loadFromFile("assets/caja.png"))                       cerr << "Error: caja.png\n";
    if (!texEstMono.loadFromFile("assets/estampilla_mono.png"))         cerr << "Error: estampilla_mono.png\n";
    if (!texEstErizo.loadFromFile("assets/estampilla_erizo.png"))       cerr << "Error: estampilla_erizo.png\n";
    if (!texEstGato.loadFromFile("assets/estampilla_gato.png"))         cerr << "Error: estampilla_gato.png\n";
}

void Renderer::cargarIcono() {
    // Usar aceituna.png como ícono de la ventana
    // Windows requiere que la imagen sea cuadrada, idealmente 32×32 o 64×64 px
    sf::Image icono;
...
void Renderer::procesarEventos(Juego& juego) {
    ...
    while (ventana.pollEvent(evento)) {
        ...
        if (est == EstadoJuego::MENU ||
            est == EstadoJuego::CREAR_CUENTA ||
            est == EstadoJuego::CONFIRMAR_ELIMINAR)
            pantallaMenu.procesarEventos(evento, juego);
        else if (est == EstadoJuego::SELECCION ||
                 est == EstadoJuego::NIVELES)
            pantallaSeleccion.procesarEventos(evento, juego);
        else if (est == EstadoJuego::JUGANDO  ||
                 est == EstadoJuego::PAUSANDO ||
                 est == EstadoJuego::GAME_OVER)
